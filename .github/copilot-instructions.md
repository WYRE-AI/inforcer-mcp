...
## Project Context
- **Stack**: TypeScript (ESM), MCP SDK (`@modelcontextprotocol/sdk`), `@wyre-technology/node-inforcer` client library
...
- Tenant resolution: tenant-scoped tools must resolve the `tenant` argument via `resolveTenantId` (numeric Client Tenant ID, DNS name, Azure AD GUID, or friendly name) before calling the API
